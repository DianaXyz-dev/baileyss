...
export const prepareWAMessageMedia = async (
	...
	// ==================== FITUR BYPASS FORK START ====================
	// Jika mendeteksi injeksi objek buatan manual yang sudah berisi jpegThumbnail / buffer tanpa file fisik,
	// langsung loloskan bentuk mentahnya agar tidak dialihkan ke proses upload WhatsApp yang merusak media interaktif.
	const inputMedia = (message as any)[mediaType]
	if (inputMedia && typeof inputMedia === 'object' && ('jpegThumbnail' in inputMedia || Buffer.isBuffer(inputMedia?.jpegThumbnail))) {
		const bypassObj = WAProto.Message.fromObject({
			[`${mediaType}Message`]: (MessageTypeProto as any)[mediaType].fromObject({
				...inputMedia,
				mimetype: inputMedia.mimetype || MIMETYPE_MAP[mediaType]
			})
		})
		return bypassObj
	}
	// ===================== FITUR BYPASS FORK END =====================
